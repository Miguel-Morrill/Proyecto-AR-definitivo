...
```{r}
AIoptimo <- c(icsGarch10[1],icsGarch11[1],icsGarch12[1],icsGarch20[1], icsGarch21[1], icsGarch22[1])
AIoptimo <- sort(AIoptimo)
AIoptimo
```
El modelo optimo para los datos es el modelo GARCH 1,1
```{r}
est11 <- garch11@h.t
```
...
L_opt <- as.numeric(L_opt[1,1])
Var_Est_EWMA <- xts(emaTA(Var_Real, lambda = L_opt), order.by= date(Rendimiento))
Resultados2 <- merge.xts(Var_Real, Var_Est_EWMA, est11)

names(Resultados2) <- c('Var_Real','Var_EWMA','est11')
plot(Resultados2, legend.loc = 'top', lwd=c(.4,.8), col=c('blue','red'))
```
...
Resultados2 <- merge.xts(Var_Real, Var_Est_EWMA, est11)
names(Resultados2) <- c('Var_Real','Var_EWMA','est11')
plot(Resultados2, legend.loc = 'top', lwd=c(.4,.8), col=c('blue','yellow', 'red'))
```
