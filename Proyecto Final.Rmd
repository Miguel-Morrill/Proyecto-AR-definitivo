...
Tiene como empresa filial a la Operadora de Sites Mexicanos, S.A. de C.V. con clave de cotización en La Bolsa Mexicana de Valores : “OSM”
  ...
  La empresa es de grandes dimensiones debido a la naturaleza de la industria en la que trabajan, y debido a que su principal fuente de ingresos es la renta de torres de telecomunicaciones, podría considerarse una empresa estable y con mucho futuro, sobre todo debido al crecimiento del mercado de telefonía móvil en México, ya que al establecerse nuevas empresas en este ámbito, tienen que utilizar infraestructura ya establecida o construir nueva.Recientemente la empresa entró al S&P/BMV IPC el pasado 21 de septiembre de 2020 teniendo un incremento en sus acciones desente del 7% antes de su entrada.
  
  Algunos riesgos de mercado a los que se podría enfrentar la empresa sería que entrase una nueva empresa de construcción de torres de telecomunicaciones y que resultara competitiva, o que la empresa no se adapte al mercado creciente del 5G y pierda el liderazgo debido a la entrada de empresas que realicen la instalación de esta tecnología en México. Otro riesgo al que se enfrenta sería que alguna de las empresas de telefonía móvil a la que le presta servicios se encuentre en problemas o quiebre, y pierda Telesites los ingresos asociados a esta empresa, sin embargo debido al tamaño del mercado que captan actualmente las empresas de telefonía móvil principalmente establecidas, las barreras de entrada son muy altas, y las que existen actualmente parece que están para quedarse.Ademas la empresa pertenece a Carlos Slim por lo que es complicado que este permita la entrada de nuevos competidores a México.
...
Hay muy poca distribucion hacia los lados.
>>>>>>> Stashed changes
...
Como se saca el rendimiento

Como sacamos la volatiidad diaria promedio, el renciemiento diario promedio y los anualizados
...
vdp_2016
vap_2016 <- sqrt(var(rendimiento[year(rendimiento)==2016])*252)
vap_2016
```
```{r}
...
vdp_2017
vap_2017 <- sqrt(var(rendimiento[year(rendimiento)==2017])*252)
vap_2017
```
...
vdp_2018
vap_2018 <- sqrt(var(rendimiento[year(rendimiento)==2018])*252)
vap_2018
```
```{r}
...
vdp_2019
vap_2019 <- sqrt(var(rendimiento[year(rendimiento)==2019])*252)
vap_2019
```
```{r}
...
vdp_2020
vap_2020 <- sqrt(var(rendimiento[year(rendimiento)==2020])*252)
vap_2020
```
```{r}
...
vdp_2021
vap_2021 <- sqrt(var(rendimiento[year(rendimiento)==2021])*252)
vap_2021
```
# Normalidad del rendimiento, del precio y del log del precio

Teoria que nosotros hemos visto sobre la normalidad del rendimiento y del los del precio

Pruebas formales de normalidad

Hay muchas pruebas de formalidad, la mas conocida y la mas usada se llama Jarque Bera
...
jbr[['statistic']]
```
Como la regla dice si el estadistico que calculamos es mayor a 9.2130 rechazamos Ho. En este caso 4112.487 es mayor a 9.2130, entonces rechazo Ho. Por lo tanto NO HAY NORMALIDAD
...
jbr_2021[['statistic']]
```
La curtosis es lo alto de la distribución
```{r}
...
Tenemor un estadistico, que se llama t de student. Si el estadistico que me de R (calculemos) es mayor a 2.3295, rechazar Ho.

Para todo el periodo vamos a tener este comportamiento
```{r}
t <- t.test(as.numeric(rendimiento))
t[['statistic']]
```
En este caso el estadistico es 0.7539823 por lo cual mi cinclusión es que no puedo rechazar Ho. 

```{r}
t_2016 <- t.test(as.numeric(rendimiento[year(rendimiento)==2016]))
...
sigma_2020
```
dS=S*mu_2020*dt+5*sigma_2020*dW
...
library(tidyverse)
```
Simular el precio dentro de 10 días
...
S0
```
Estimancion para el precio del día 25 febrero + 10 días de cotización
...
Z <- qnorm(.975)
P_Sup <- S0+Media_DS+Z*DeSt_DS
P_Inf <- S0+Media_DS-Z*DeSt_DS
P_Sup
P_Inf
S0
```
qnorm es irte adentro de la tabla
=======
